...
@section('title','Renouveler l\'inscription')
@section('content') 
...
<a href="{{url('/inscriptions/')}}" title="Retour">
  <i class="align-middle me-2" data-feather="arrow-left-circle"></i>  
     ...
     <strong>Renouveler l'inscription </strong>
</h4>        

<form method="post" action="{{url('/renouvelement/'.$inscription->id)}}"  >
{{ csrf_field() }}  
                ...
                <div class="col-sm-8">
                     ...
                     @foreach($années as $année)
                            @if($inscription->annee_id == $année->id)
                                  <option value="{{ $année->id }}" selected disabled>{{ $année->titre }}</option>
                              @else
                     ...
                     <select class="form-select mb-3" name="classe_id" title="Classe" required>  
                      @foreach($classes as $classe)
                        @if($inscription->classe_id == $classe->id)
                            <option value="{{ $classe->id }}" selected disabled>{{ $classe->categories->titre }} => {{ $classe->niveaus->titre }} => {{ $classe->titre }}</option>
                        @else
